...
SVDResult dumb_warm_rsvd(const MatF& A, const MatF* U_prev,
    ...
    // -----------------------------------------------------------------------
    // 1. Random exploration sketch  Omega ~ N(0,1)  shape (n, p)
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // 2. Random matmul  Y2 = A @ Omega   shape (m, p)
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // 3. Concatenate  Y = [U_prev, Y2]   shape (m, r_prev + p)
    //    U_prev is copied directly — no matmul with A.
    // -----------------------------------------------------------------------
    timer.start("concat");
    MatF Y(m, sketch);
    Y.leftCols(r_prev)  = *U_prev;
    Y.rightCols(p)      = Y2;
    timer.stop("concat");

    // -----------------------------------------------------------------------
    // 4. Power iterations  (applied to full Y, same as warm_rsvd)
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // 5. QR decomposition  Q shape (m, sketch)
    // -----------------------------------------------------------------------
    timer.start("qr");
    ...
    // -----------------------------------------------------------------------
    // 6. Projection  B = Q.T @ A   shape (sketch, n)
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // 8. Lift  U = Q @ Uhat   shape (m, sketch)
    // -----------------------------------------------------------------------
    timer.start("lift");
    MatF U_full = Q * Uhat;
    timer.stop("lift");
    ...
    // warm_proj and warm_matmul are intentionally absent (skipped steps)

    // Matmul counts: same as cold (one AX + one ATX at q=0)
    res.matmuls["AX"]  = 1 + q;   // random_matmul + q*(A@Z)
    res.matmuls["ATX"] = 1 + q;   // projection + q*(A.T@Y)
